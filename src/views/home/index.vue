...
export default {
  ...
  methods: {
    ...
    // 1. 使用组件注意：绑定原生事件的时候注意组件是否支持这个事件
    ...
    handler (command) {
      ...
      // 如果是 setting 跳转去个人设置  如果是logout 清除用户信息跳转去登录
      // if (command == 'setting')
      // if (command == 'logout')
...
.container-home {
  ...
  width: 100%;
  height: 100%;
  //elemengt-ui提供的组件 解析后再容器上追加一个和组件同名的class
  .el-aside {
    background-color: #002033;
    .logo {
      width: 100%;
      height: 60px;
      background: #002244 url(../../assets/assets/logo_admin.png) no-repeat
        center / 140px auto;
    }
    //将来覆盖logo的样式
    .minLogo {
       background-image: url(../../assets/assets/logo_admin_01.png);
